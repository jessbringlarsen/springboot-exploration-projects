...
@Component
public class DepositMoneyUseCase extends AbstractUseCase<DepositMoneyUseCase.Input, Transaction> {

    private final TransactionRepository transactionRepository;
    private final AccountRepository accountRepository;

    @Autowired
    public DepositMoneyUseCase(Validator validator,
    ...
    public Transaction doExecute(Input input) {
        Optional<Account> account = accountRepository.findById(input.getTransaction().accountId());
        if (account.isEmpty()) {
            throw new UseCaseValidationException("Account not found.");
        }
        return transactionRepository.persist(input.getTransaction());
    }
